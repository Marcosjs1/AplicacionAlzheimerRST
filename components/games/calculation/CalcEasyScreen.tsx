...
const GAME_NAME = "Suma y Resta Fácil";
const GAME_TYPE = "calculation_easy";
...
const CalcEasyScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // (Opcional) Insertar el juego si no existe (dev-friendly)
        if (!finalGameId) {
          const { data: created, error: createErr } = await supabase
            ...
            .insert([
              {
                name: GAME_NAME,
                description: "Ejercicios simples de suma y resta con números pequeños.",
        ...
        console.error("Error init CalcEasy:", e);
  ...
  const generateQuestion = (id: number): Question => {
    ...
    return { id, a, b, operator, correctAnswer, options };
  };
  ...
  const handleAnswer = (option: number) => {
    ...
    setTimeout(() => {
      ...
      if (currentIndex + 1 < TOTAL_QUESTIONS) {
        ...
        // ✅ pasar valores finales para evitar desync
        handleGameComplete({
          finalHits: newScore,
  ...
  const handleGameComplete = async ({
    ...
    try {
      ...
      const { error: levelErr } = await supabase.from("game_level_results").insert([
        {
          session_id: sid,
          level: 1,
      ...
      const { error: progErr } = await supabase.from("game_progress").insert([
        {
          user_id: user.id,
          game_type: GAME_TYPE,
          score: finalHits,
          details: {
            ...
            accuracy: Number(accuracy.toFixed(1)), // ✅ número
  ...
  const finalizeAbandonedSession = async () => {
    ...
    try {
      ...
      // Insert level partial result (opcional)
  ...
  const handleExit = async () => {
    if (isActive && !isGameComplete) {
      ...
      // ✅ Guardar parcial antes de salir
  ...
  // fallback si todavía no se seteó
        ...
        {!isGameComplete ? (
          <div className="w-full max-w-md animate-in fade-in duration-500">
            ...
            <div className="bg-white dark:bg-surface-dark rounded-[32px] p-8 shadow-xl shadow-primary/5 border border-primary/5 mb-8 text-center relative overflow-hidden">
              ...
              <div className="flex items-center justify-center gap-4 text-[64px] font-black text-slate-900 dark:text-white leading-none">
                <span>{currentQ.a}</span>
                <span className="text-primary">{currentQ.operator}</span>
            ...
            <div className="grid gap-4">
              {currentQ.options.map((option, idx) => (
                <button
                  ...
                  className={`
                    w-full py-6 rounded-2xl text-3xl font-bold transition-all transform active:scale-[0.98]
                    ${
          ...
          <div className="w-full max-w-md bg-white dark:bg-surface-dark rounded-[32px] p-8 shadow-2xl border border-primary/10 text-center animate-in zoom-in-95 duration-500">
            ...
            <p className="text-slate-500 dark:text-slate-400 mb-8 font-medium">
              Excelente trabajo manteniendo tu mente activa.
            </p>
