...
const GAME_NAME = "Toca el Objetivo (Fácil)";
const GAME_TYPE = "attention_easy";
...
const AttentionEasyScreen: React.FC = () => {
  ...
  // ✅ attempts reales (solo cuenta taps)
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
            ...
            .insert([
              {
                name: GAME_NAME,
                description:
                  "Toca la estrella ⭐ lo más rápido posible. Entrena atención visual.",
        ...
        console.error("Error init AttentionEasy:", e);
  ...
  const generateRoundGrid = () => {
    ...
    for (let i = 0; i < 9; i++) {
      if (i === targetPos) {
        newGrid.push({ id: i, content: TARGET_ICON, isTarget: true });
      } else {
        const distractor = shuffledDistractors[i % shuffledDistractors.length];
        newGrid.push({ id: i, content: distractor, isTarget: false });
      }
    }
  ...
  const handleTap = (item: GridItem) => {
    ...
    // ✅ contar intentos reales cuando toca
...
export default AttentionEasyScreen;